# Hi, I'm Aniket Mishra 👋

## Data Analyst | Business Insights | Data Science

I am a **Data Analyst** with a strong focus on transforming raw data into meaningful business insights that support strategic and operational decision-making. I work closely with stakeholders and managers to translate business questions into data-driven solutions using analytics, visualization, and machine learning techniques.

My expertise lies in **data analysis, reporting, dashboarding, and predictive modeling**, with a clear emphasis on clarity, accuracy, and business impact.

---

## Skills

### Data Analysis & Analytics
- Data Cleaning, Data Validation, and Data Quality Checks
- Exploratory Data Analysis (EDA)
- Business Metrics & KPI Development
- Statistical Analysis
- Predictive & Descriptive Analytics
- Data Visualization
- Model Building & Optimization

### Programming & Scripting Languages
- **Python**
- **R**
- **C++**

### Databases
- **MySQL**
- **OracleSQL**

### Data Visualization Tools
- **Power BI**
- **Tableau**
- **Advanced Microsoft Excel** (Pivot Tables, Power Query, VBA basics, Dashboards)

### Data Analysis Tools
- Excel (Advanced Functions, Lookups, Macros)
- Power BI Service
- Jupyter Notebook
- Google Colab

---

## Tools & Technologies

### Python Libraries
- NumPy
- Pandas
- Matplotlib
- Seaborn
- Plotly
- Scikit-learn
- Statsmodels
- SciPy
- BeautifulSoup
- Requests
- PyTorch
- OpenCV
- Tensorflow

### Machine Learning & AI Models

**Supervised Learning**
- Linear Regression
- Multiple Linear Regression
- Polynomial Regression
- Logistic Regression
- Decision Tree
- Random Forest
- Gradient Boosting
- XGBoost
- LightGBM
- K-Nearest Neighbors (KNN)
- Support Vector Machine (SVM)
- Naive Bayes

**Unsupervised Learning**
- K-Means Clustering
- Hierarchical Clustering
- DBSCAN
- Principal Component Analysis (PCA)

**Time Series & Forecasting**
- ARIMA
- Exponential Smoothing

**Model Evaluation & Optimization**
- Cross Validation
- Grid Search
- Random Search
- ROC-AUC, Precision, Recall, F1-Score

---

## What I Bring to the Table
- Strong analytical mindset with a business-first approach
- Clear and concise communication for managers and stakeholders
- End-to-end ownership of data analysis projects
- Ability to convert complex data into simple, actionable insights

---

## Connect With Me
- **GitHub:** https://github.com/aniket622
- **LinkedIn:** https://linkedin.com/in/aniketmishra622
- **Email:** aniketmishra622@gmail.com 
